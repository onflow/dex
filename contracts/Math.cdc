...
Math provides some math utility functions.
...
pub contract Math {
    ...
    access(contract) let scaleUI256: UInt256
    /// fixed scaling factor 10^8 in UFix64
    access(contract) let scaleUF64: UFix64
    ...
    pub fun addressToUInt64(address: Address): UInt64 {
        ...
        assert(addressBytes.length == 8, message: "Math: Address should be 64 bits")
